"""Loader for UST USFM files with section-based chunking.

Parses USFM from datasets/ust, chunks verses using the same logic as
the BSB loader (OpenBible section counts), and posts documents to the
servant engine collection "ust".
"""
...
DATASET_DIR = Path(__file__).resolve().parent / "datasets" / "ust"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Load UST USFM into servant, or print chunks")
    parser.add_argument(
        "--print-chunks-only",
        action="store_true",
        dest="print_only",
    ...
    logger.info("Parsed %d verses from %d UST books", len(verses), len(files))

    chunks = group_semantic_chunks(verses, include_text=True)
    logger.info("Prepared %d UST chunks", len(chunks))

    documents = _build_documents(chunks, collection="ust")
    if args.print_only:
        # Print JSON for inspection (no network I/O)
    ...
    logger.info("UST insertion complete: %d success, %d failed", ok, fail)
